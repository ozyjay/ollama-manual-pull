...
class MacOSAppBuilderTests(unittest.TestCase):
    def test_build_app_creates_pyenv_aware_bundle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("store.selectedId = activeItem.id", combined_source)
            self.assertNotIn("progress.currentFile ?? progress.overall", combined_source)
            self.assertIn("private var currentFile: ProgressAmount?", combined_source)
            self.assertIn("Current blob", combined_source)
            ...
            self.assertIn("/api/stop-after-blob", combined_source)
            self.assertIn("canStopAfterBlob", combined_source)
            self.assertIn("Stop After Blob", combined_source)
            ...
            self.assertIn("clearActionError(prefix: \"Stop after blob failed:\")", combined_source)
            ...
            self.assertIn("selectedSourceId", combined_source)
            self.assertIn("Picker(\"Source\"", combined_source)
            self.assertIn("func search(_ query: String, sourceId: String) async throws", combined_source)
            ...
            self.assertIn("/api/cleanup/scan", combined_source)
            self.assertIn("/api/cleanup/delete", combined_source)
            self.assertIn("func scanCleanup", combined_source)
            self.assertIn("func deleteCleanupCandidates", combined_source)
            self.assertIn("struct CleanupReport", combined_source)
            self.assertIn("Scan Orphan Shards", combined_source)
            self.assertIn("Include stale partials", combined_source)
            self.assertIn("Shared blobs are kept", combined_source)
