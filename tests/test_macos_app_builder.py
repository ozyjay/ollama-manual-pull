...
class MacOSAppBuilderTests(unittest.TestCase):
    def test_build_app_creates_pyenv_aware_bundle(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertTrue((resources / "NativeApp.swift").is_file())
            ...
            source_text = (resources / "NativeApp.swift").read_text()
            self.assertIn("/Users/example/.pyenv/versions/3.12.13/bin/python3", source_text)
            self.assertIn("NSHostingView", source_text)
            self.assertIn("URLSession", source_text)
            self.assertIn("Process", source_text)
            self.assertIn("ollama_manual_pull.server", source_text)
            self.assertIn("create_server(('127.0.0.1', 0)", source_text)
            self.assertLess(source_text.index("createWindow()"), source_text.index("startServer()"))
            self.assertNotIn("WKWebView", source_text)
            self.assertNotIn("WebKit", source_text)
